	...
	include 'db_connect.php';
		...
		if(empty($id)){
				...
				$e_num=date('Y') .'-'. mt_rand(1,9999);
					$chk  = $conn->query("SELECT * FROM employee where employee_id = '$id' ")->num_rows;
					if($chk <= 0){
						$i = 0;
					}
				}
				// echo "INSERT INTO `employee` VALUES('', '$e_num','$firstname', '$middlename', '$lastname', '$position')";
				// exit;
				$save=$conn->query("INSERT INTO `employee` VALUES('', '$firstname', '$middlename', '$lastname', '$position')") or die(mysqli_error());
				if($save){
